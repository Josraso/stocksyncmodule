...
class AdminStockSyncLogsController extends ModuleAdminController
{
    ...
    public function getListQuery()
    {
        ...
        // Asegurarse de que la tabla de colas existe antes de hacer un join
        if ($this->tableExists(_DB_PREFIX_ . 'stock_sync_queue')) {
            // Usar IFNULL para evitar problemas si la referencia es NULL
            $this->_select = 'a.*, IFNULL(q.reference, "") AS queue_reference';
            $this->_join = 'LEFT JOIN `' . _DB_PREFIX_ . 'stock_sync_queue` q ON q.id_queue = a.id_queue';
        } else {
            $this->_select = 'a.*, "" AS queue_reference';
        }
