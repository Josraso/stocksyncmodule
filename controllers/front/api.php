...
class StocksyncmoduleApiModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        try {
            ...
            if (class_exists('StockSyncTools')) {
                // ARREGLO: mejor validación de token
                $validToken = StockSyncTools::validateToken($token);
                
                // Si el token es inválido, probemos a ver si es un token sin timestamp
                if (!$validToken && strpos($token, '.') === false) {
                    // Revisemos todas las tiendas activas para ver si coincide con alguna API key
    ...
    private function returnJson($data)
    {
        ...
        if (Configuration::get('STOCK_SYNC_DEBUG_MODE')) {
            ...
            file_put_contents(
                $log_file, 
                date('Y-m-d H:i:s') . ' - Response: ' . print_r($data, true) . "\n", 
                FILE_APPEND
            );
        }
