...
class StockSyncReference
{
    ...
    public static function scanAndMapReferences($source_store_id, $target_store_id, $force = false)
    {
        ...
        // Verificar que exista la tabla de referencias
        if (!self::tableExists(_DB_PREFIX_ . 'stock_sync_references')) {
            error_log('ERROR: La tabla de referencias no existe');
            return $results;
        }
        
        // IMPORTANTE: Eliminamos cualquier validación que impida mapear cuando source_store_id == target_store_id
        // o cuando los IDs de productos son iguales. Esto nos permite mapear entre tiendas clonadas.
        
        try {
            // Paso 1: Obtener todos los productos con referencias
            $products_query = '
                SELECT 
                    p.id_product, 
                    p.reference, 
            ...
            // Paso 2: Para cada producto, crear directamente un mapeo
            // Asumimos que estamos trabajando con tiendas clonadas con los mismos productos
            foreach ($products as $product) {
                if (empty($product['reference'])) {
                    continue;
                }
                
                $existing = self::getReferenceMapping($product['reference'], $source_store_id, $target_store_id);
                
                if ($existing && !$force) {
                    // Ya existe el mapeo y no estamos forzando actualización
                    continue;
                }
                
                // Crear mapeo directo (asumiendo mismos IDs en ambas tiendas)
                $result = self::addReferenceMapping(
                    $product['reference'],
                    $product['id_product'],
                    $product['id_product_attribute'],
                    $product['id_product'], // Mismo ID en tienda destino
                    $product['id_product_attribute'], // Mismo ID en tienda destino
                    $source_store_id,
                    $target_store_id
                );
...
public static function getStatistics()
{
    ...
    try {
        ...
        // Contar total directamente
        $query = "SELECT COUNT(*) as total FROM `" . $table_name . "`";
        $result = Db::getInstance()->getValue($query);
        $stats['total'] = (int)$result;
        
        // Verificar si la columna 'active' existe
        $column_exists = Db::getInstance()->executeS("SHOW COLUMNS FROM `" . $table_name . "` LIKE 'active'");
        if (!empty($column_exists)) {
            // Contar activos
            $query = "SELECT COUNT(*) FROM `" . $table_name . "` WHERE active = 1";
            $stats['active'] = (int)Db::getInstance()->getValue($query);
            $stats['inactive'] = $stats['total'] - $stats['active'];
        }
        
        // Verificar si la columna 'id_product_attribute_source' existe
        $column_exists = Db::getInstance()->executeS("SHOW COLUMNS FROM `" . $table_name . "` LIKE 'id_product_attribute_source'");
        if (!empty($column_exists)) {
            // Contar productos y combinaciones
            $query = "SELECT COUNT(*) FROM `" . $table_name . "` WHERE id_product_attribute_source = 0";
            $stats['products'] = (int)Db::getInstance()->getValue($query);
            
            $query = "SELECT COUNT(*) FROM `" . $table_name . "` WHERE id_product_attribute_source > 0";
            $stats['combinations'] = (int)Db::getInstance()->getValue($query);
        }
        
        // Loguear para diagnóstico
        error_log('Reference Stats Result: ' . print_r($stats, true));
