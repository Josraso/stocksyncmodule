...
class StockSyncReference
{
    ...
    public static function getStatistics()
    {
        ...
        $query = new DbQuery();
        $query->select('active, COUNT(*) as count')
            ->from('stock_sync_references')
            ->groupBy('active');
        ...
        // Get product/combination counts
        $query = new DbQuery();
        $query->select('COUNT(*) as count')
            ->from('stock_sync_references')
            ->where('id_product_attribute_source = 0');
        ...
        $result = Db::getInstance()->getRow($query);
        if ($result) {
            $stats['combinations'] = (int) $result['count'];
        }
        
        // Get newest/oldest
        $query = new DbQuery();
        $query->select('MIN(created_at) as oldest, MAX(created_at) as newest')
            ->from('stock_sync_references');
