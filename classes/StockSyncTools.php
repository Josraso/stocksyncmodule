...
class StockSyncTools
{
    ...
    public static function generateTokenForRequest($api_key)
    {
        ...
        if (isset(self::$tokenCache[$cache_key]) && (time() - self::$tokenCache[$cache_key]['time']) < 3600) {
            return self::$tokenCache[$cache_key]['token'];
        }
        ...
        $timestamp = time();
        $token = hash('sha256', $api_key . $salt . $timestamp);
        $fullToken = $token . '.' . $timestamp;
        
        // Cache token for one hour to improve performance
        self::$tokenCache[$cache_key] = [
            'token' => $fullToken,
            'time' => time()
        ];
    ...
    public static function validateToken($token, $api_key = null)
    {
        ...
        $expiry = (int) Configuration::get('STOCK_SYNC_TOKEN_EXPIRY', 86400); // Default: 1 day
        ...
        if ($api_key) {
            // Check against specific API key
            $expected_hash = hash('sha256', $api_key . $salt . $timestamp);
            return $hash === $expected_hash;
        } else {
            // Optimización: limitar el número de tiendas a verificar
            $stores = StockSyncStore::getActiveStores();
            
            // No consultar todas las tiendas, limitar a 10 como máximo
