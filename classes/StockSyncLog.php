...
class StockSyncLog
{
    ...
    public static function getRecent($limit = 100, $level = '')
    {
        ...
        $query = new DbQuery();
        $query->select('l.*, IFNULL(q.reference, "") as reference')
        ...
        if ($level && self::columnExists('stock_sync_log', 'level')) {
            ...
            $query->where('l.level IN (' . implode(',', $levels_sql) . ')');
        }
...
public static function getStatistics($hours = 0)
{
    ...
    try {
        ...
        $column_exists = Db::getInstance()->executeS("SHOW COLUMNS FROM `" . $table_name . "` LIKE 'level'");
        if (!empty($column_exists)) {
            // Contar por nivel
            $niveles = ['info', 'warning', 'error', 'conflict'];
            foreach ($niveles as $nivel) {
                $query = "SELECT COUNT(*) FROM `" . $table_name . "` WHERE level = '" . pSQL($nivel) . "'";
                $count = Db::getInstance()->getValue($query);
                $stats[$nivel] = (int)$count;
            }
        }
        
        // Loguear para diagnóstico
        error_log('Log Stats Result: ' . print_r($stats, true));
