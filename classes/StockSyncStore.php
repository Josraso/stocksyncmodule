...
public static function getStatistics()
{
    ...
    try {
        ...
        // Verificar si la columna 'sync_type' existe
        $column_exists = Db::getInstance()->executeS("SHOW COLUMNS FROM `" . $table_name . "` LIKE 'sync_type'");
        if (!empty($column_exists)) {
            // Contar por tipo
            $tipos = ['principal', 'secundaria', 'bidirectional'];
            foreach ($tipos as $tipo) {
                $query = "SELECT COUNT(*) FROM `" . $table_name . "` WHERE sync_type = '" . pSQL($tipo) . "'";
                $count = Db::getInstance()->getValue($query);
                $stats[$tipo] = (int)$count;
            }
        }
        
        // Loguear para diagnóstico
        error_log('Store Stats Result: ' . print_r($stats, true));
