...
class StockSyncStore extends ObjectModel
{
    ...
    public static function getStatistics()
    {
        ...
        foreach ($stores as $store) {
            ...
            $stats[$store['sync_type']]++;
            
            if (!$stats['newest'] || strtotime($store['created_at']) > strtotime($stats['newest'])) {
                $stats['newest'] = $store['created_at'];
            }
            
            if (!$stats['oldest'] || strtotime($store['created_at']) < strtotime($stats['oldest'])) {
                $stats['oldest'] = $store['created_at'];
            }
        }
