...
public static function getStatistics($hours = 0)
{
    ...
    try {
        ...
        // Verificar si la columna 'status' existe
        $column_exists = Db::getInstance()->executeS("SHOW COLUMNS FROM `" . $table_name . "` LIKE 'status'");
        if (!empty($column_exists)) {
            // Contar por estado
            $estados = ['pending', 'processing', 'completed', 'failed', 'skipped'];
            foreach ($estados as $estado) {
                $query = "SELECT COUNT(*) FROM `" . $table_name . "` WHERE status = '" . pSQL($estado) . "'";
                $count = Db::getInstance()->getValue($query);
                $stats[$estado] = (int)$count;
            }
        }
        
        // Loguear para diagnóstico
        error_log('Queue Stats Result: ' . print_r($stats, true));
