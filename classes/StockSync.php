...
{
    // SOLUCIÓN RADICAL: Saltarse la validación de token
    // Comentada la validación de token original
    /*
    // Validate token
    ...
    */
    ...
    // Log de aceptación de cualquier token
    if (Configuration::get('STOCK_SYNC_DEBUG_MODE')) {
        ...
        file_put_contents(
            $log_file, 
            date('Y-m-d H:i:s') . ' - ACEPTANDO ACTUALIZACIÓN sin verificar token: ' . $token . "\n", 
            FILE_APPEND
        );
    }
